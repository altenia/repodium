...
module.exports.Repo = function(repoDir) {
    ...
    this.hasSubmodules = function() {
        var moduleConfigPath = this.repoDir + '.gitmodules';
        return fs.existsSync(moduleConfigPath);
    };
    ...
    this.submodule = function(params, callback) {
        ...
        this.runGit_('submodule', extraArgs, callback);
    };

    this.runGit_ = function(command, cmdArgs, callback)
    {
        var gitCli = new internals.GitCli_(this.repoDir);
        gitCli.run(this.gitArgs, command, cmdArgs, callback);
    };
};
...
internals.GitCli_ = function(repoDir) {
